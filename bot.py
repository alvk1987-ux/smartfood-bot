...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # === ОБНОВЛЕННЫЙ ТЕКСТ ДЛЯ КНОПКИ ===
    if text == "📖 Как общаться с Шефом":
        guide_text = (
            "👨‍🍳 <b>Гайд: Как выжать максимум из «Премиального Шефа»? Описание всех функций!</b>\n\n"
            "Друзья, добро пожаловать! Мой бот — это не просто поисковик рецептов, это ваш личный виртуальный шеф-повар, диетолог и умный помощник на кухне.\n\n"
            "Чтобы вы могли создавать кулинарные шедевры легко и быстро, я подготовил для вас инструкцию по всем кнопкам меню. 👇\n\n"
            "➖➖➖➖➖➖➖➖➖➖\n"
            "📱 <b>ГЛАВНОЕ МЕНЮ (Поиск и создание рецептов)</b>\n"
            "➖➖➖➖➖➖➖➖➖➖\n\n"
            "<b>🔍 Найти рецепт</b>\n"
            "Самая главная кнопка. Мой бот отлично понимает человеческую речь, поэтому общайтесь с ним, как с настоящим живым шеф-поваром! Чем подробнее запрос, тем круче результат.\n"
            "💡 <i>Пример запроса: «Напиши рецепт сочной говядины в духовке, чтобы была с корочкой, и сделай к ней необычный сливочно-чесночный соус».</i>\n\n"
            "<b>🧺 Из того, что есть</b>\n"
            "Спасение, когда лень идти в магазин. Откройте холодильник и перечислите боту всё, что там завалялось. Он придумает, как это вкусно совместить!\n"
            "💡 <i>Пример запроса: «Куриное филе, половинка луковицы, немного засохший сыр, сметана и макароны».</i>\n\n"
            "<b>⚡ Быстрый ужин</b>\n"
            "Когда вы пришли с работы, очень голодны, а времени стоять у плиты нет. Бот выдаст рецепт, который готовится не дольше 15-20 минут.\n"
            "💡 <i>Пример запроса: «Что-то из индейки» или «Паста без мяса».</i>\n\n"
            "<b>🥗 Рецепты для похудения</b>\n"
            "Идеально для тех, кто следит за фигурой. Бот подберет низкокалорийные блюда и обязательно распишет точное КБЖУ.\n"
            "💡 <i>Пример запроса: «Легкий ужин без углеводов» или «Что приготовить из творога на завтрак на массе».</i>\n\n"
            "<b>📸 Калории по фото</b>\n"
            "Магия вне Хогвартса! Нажмите эту кнопку, а затем просто отправьте боту фотографию вашей тарелки с едой.\n"
            "💡 <i>Как работает: Бот сам распознает блюдо на фото и выдаст примерный расчет Калорий, Белков, Жиров и Углеводов. Идеально для походов в ресторан или кафе!</i>\n\n"
            "➖➖➖➖➖➖➖➖➖➖\n"
            "💾 <b>ВАША ЛИЧНАЯ БАЗА (Списки и сохранения)</b>\n"
            "➖➖➖➖➖➖➖➖➖➖\n\n"
            "<b>🛒 Мой список покупок</b>\n"
            "Сюда попадают продукты из рецептов. Вы открываете этот список в супермаркете и ничего не забываете купить! Внутри есть кнопка «🗑 Очистить список», чтобы удалять купленное.\n\n"
            "<b>⭐ Сохраненные рецепты</b>\n"
            "Ваша личная кулинарная книга. Все рецепты, которые вам понравились и вы захотели их сохранить, лежат здесь.\n\n"
            "<b>👑 Моя подписка</b>\n"
            "Здесь можно проверить статус вашего VIP-доступа, а также безопасно оплатить его через Робокассу.\n\n"
            "➖➖➖➖➖➖➖➖➖➖\n"
            "🕹 <b>КНОПКИ ПОД РЕЦЕПТОМ (Магия управления)</b>\n"
            "➖➖➖➖➖➖➖➖➖➖\n"
            "Когда Шеф выдал вам готовый рецепт, под ним появляются 4 уникальные кнопки. Вот что они делают:\n\n"
            "🟢 <b>🛒 В список покупок</b>\n"
            "Нажали? Бот мгновенно вытащит все ингредиенты из этого рецепта (грамм в грамм) и закинет их в ваш «Список покупок». Больше не нужно переписывать продукты на бумажку!\n\n"
            "🟡 <b>⭐ Сохранить рецепт</b>\n"
            "Понравилось блюдо? Одно нажатие, и рецепт навсегда сохранен в разделе «Сохраненные рецепты» главного меню.\n\n"
            "🔵 <b>🔄 Заменить продукт</b>\n"
            "Рецепт классный, но у вас аллергия на грибы? Или вы ненавидите лук? Нажмите эту кнопку и скажите боту, что нужно исправить.\n"
            "💡 <i>Пример: Выпал рецепт супа. Вы нажимаете «Заменить продукт» и пишете: «Убери из рецепта лук и замени обычное молоко на кокосовое». Бот тут же перепишет рецепт специально под вас!</i>\n\n"
            "⚪️ <b>🎲 Другой вариант</b>\n"
            "Если блюдо вам просто не приглянулось, нажмите эту кнопку. Бот сгенерирует абсолютно новое блюдо по вашему изначальному запросу.\n\n"
            "👨‍🍳 Экспериментируйте, готовьте с удовольствием и делитесь фотографиями ваших блюд в моем чате! Приятного аппетита!"
        )
